...
use App\GameEngine\Battleship\{Ship, BattleshipP, BattleshipQ};
...
class BattleshipGame
{
     ...
     * @param int $startPintP   Coordinate of P
     * @param int $startPointQ  Coordinate of Q
     *
     * @return BattleshipGame
     */
    public static function initCustomGame($rows, $columns, $startPintP, $startPintQ)
    {
        //return new BattleshipGame($rows, $columns);
        return new BattleshipGame($rows, $columns, $startPintP, $startPintQ);
    }
    ...
    public static function resumeGame($layout)
    {
        ...
        //$game = new BattleshipGame($rows, $columns, $startPintP, $startPintQ);
     ...
     * @param int $startPointQ  Coordinate of Q
     */
    private function __construct($rows, $columns, $startPintP='', $startPintQ='')
    {
        $this->board = new Board($rows, $columns);
        //$this->initShips();
        $this->initBattleships($startPintP, $startPintQ);
    }
     ...
     */
    //private function initBattleships()
    private function initBattleships($startPintP, $startPintQ)
    { 
        $this->placeBattleshipOnBoard(new BattleshipP(), $startPintP);
        $this->placeBattleshipOnBoard(new BattleshipQ(), $startPintQ);
    }
     ...
     *
     * @param Ship $ship   Instance of ship to place
     * @param string $location start co-ordinate e.g 'A5'
     */
    private function placeBattleshipOnBoard(Ship $ship, $location)
    {

        // Register new ships on the board
        // so we will know when the game ends
        $sLength = $ship->getLength();
        $extra = ['id' => $ship->getType(), 'size' => $sLength];
        ...
        $pars['direction'] = 1;
        $direction = new Direction($pars['direction']);
        $params = new DirectedPlacingParams($row, $col, self::POINT_SHIP, $length, $direction, $extra);
    ...
    /**
     * Generate game board layout with ships
     *  from ordinary layout
     *
     * @return array
     */
    public function generateGameLayoutWithShips()
    {
        $board = $this->board;
        $board->loopLayout(function($val, $row, $col) use (&$layout) {

            if ($val === self::POINT_NO_SHOT) {

                $layout[$row][$col] = '.';

            } elseif($val === self::POINT_SHIP){

                    $layout[$row][$col] = self::POINT_SHIP;
